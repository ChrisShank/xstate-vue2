import { shallowRef, isRef, watch, onMounted, onBeforeUnmount, Ref } from '@vue/composition-api';
import { createMachine, interpret, StateMachine, EventObject, Typestate } from '@xstate/fsm';
...
export function useMachine<TContext extends object, TEvent extends EventObject = EventObject>(
	...
	const state = shallowRef<StateMachine.State<TContext, TEvent, any>>(getServiceValue(service));
...
>(
	...
	const serviceRef: Ref<StateMachine.Service<TContext, TEvent, TState>> = isRef(service)
		? service
		: shallowRef(service);
	const state = shallowRef<StateMachine.State<TContext, TEvent, TState>>(serviceRef.value.state);
